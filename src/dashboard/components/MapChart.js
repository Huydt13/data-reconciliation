// import React from 'react';
// import {
//   ComposableMap,
//   Geographies,
//   Geography,
//   Marker,
// } from 'react-simple-maps';
// import styled from 'styled-components';
// import { Card } from 'semantic-ui-react';

// const geoUrl =
//   'https://raw.githubusercontent.com/zcreativelabs/react-simple-maps/master/topojson-maps/world-110m.json';

// const markers = [
//   {
//     level: 10,
//     markerOffset: -15,
//     name: 'Ha Noi',
//     coordinates: [21.028511, 105.804817],
//   },
//   {
//     level: 30,
//     markerOffset: -15,
//     name: 'Ho Chi Minh',
//     coordinates: [10.762622, 106.660172],
//   },
//   // {
//   //   level: 5,
//   //   markerOffset: 25,
//   //   name: 'Brasilia',
//   //   coordinates: [-47.8825, -15.7942],
//   // },
//   // {
//   //   level: 7,
//   //   markerOffset: 25,
//   //   name: 'Santiago',
//   //   coordinates: [-70.6693, -33.4489],
//   // },
//   // {
//   //   level: 5,
//   //   markerOffset: 25,
//   //   name: 'Bogota',
//   //   coordinates: [-74.0721, 4.711],
//   // },
//   // {
//   //   level: 5,
//   //   markerOffset: 25,
//   //   name: 'Quito',
//   //   coordinates: [-78.4678, -0.1807],
//   // },
//   // {
//   //   level: 5,
//   //   markerOffset: -15,
//   //   name: 'Georgetown',
//   //   coordinates: [-58.1551, 6.8013],
//   // },
//   // {
//   //   level: 7,
//   //   markerOffset: -15,
//   //   name: 'Asuncion',
//   //   coordinates: [-57.5759, -25.2637],
//   // },
//   // {
//   //   level: 10,
//   //   markerOffset: 25,
//   //   name: 'Paramaribo',
//   //   coordinates: [-55.2038, 5.852],
//   // },
//   // {
//   //   level: 3,
//   //   markerOffset: 25,
//   //   name: 'Montevideo',
//   //   coordinates: [-56.1645, -34.9011],
//   // },
//   // {
//   //   level: 20,
//   //   markerOffset: -15,
//   //   name: 'Caracas',
//   //   coordinates: [-66.9036, 10.4806],
//   // },
//   // {
//   //   level: 6,
//   //   markerOffset: -15,
//   //   name: 'Lima',
//   //   coordinates: [-77.0428, -12.0464],
//   // },
// ];

// const StyledCard = styled(Card)`
//   width: auto !important;
//   padding-top: 8px;
// `;

// const MapChart = () => {
//   return (
//     <div>
//       <StyledCard>
//         <Card.Header textAlign="center" content="Bản đồ nhiệt" />
//         <ComposableMap
//           projection="geoAlbers"
//           projectionConfig={{
//             rotate: [0, 20, 0],
//             scale: 200,
//           }}
//         >
//           <Geographies geography={geoUrl}>
//             {({ geographies }) =>
//               geographies
//                 .filter((d) => (d.properties.NAME === 'Vietnam'))
//                 .map((geo) => (
//                   <Geography
//                     key={geo.rsmKey}
//                     geography={geo}
//                     fill="#EAEAEC"
//                     stroke="#D6D6DA"
//                   />
//                 ))
//             }
//           </Geographies>
//           {markers.map(({ level, name, coordinates, markerOffset }) => (
//             <Marker key={name} coordinates={coordinates}>
//               <circle r={level} fill="#F00" stroke="#fff" strokeWidth={2} />
//               <text
//                 textAnchor="middle"
//                 y={markerOffset}
//                 style={{ fontFamily: 'system-ui', fill: '#5D5A6D' }}
//               >
//                 {name}
//               </text>
//             </Marker>
//           ))}
//         </ComposableMap>
//       </StyledCard>
//     </div>
//   );
// };

// export default MapChart;
import React from 'react';
import {
  ComposableMap,
  Geographies,
  Geography,
  Marker,
} from 'react-simple-maps';
import styled from 'styled-components';
import { Card } from 'semantic-ui-react';

const geoUrl =
  'https://raw.githubusercontent.com/zcreativelabs/react-simple-maps/master/topojson-maps/world-110m.json';

const markers = [
  {
    level: 10,
    markerOffset: -15,
    name: 'Buenos Aires',
    coordinates: [-58.3816, -34.6037],
  },
  {
    level: 30,
    markerOffset: -15,
    name: 'La Paz',
    coordinates: [-68.1193, -16.4897],
  },
  {
    level: 5,
    markerOffset: 25,
    name: 'Brasilia',
    coordinates: [-47.8825, -15.7942],
  },
  {
    level: 7,
    markerOffset: 25,
    name: 'Santiago',
    coordinates: [-70.6693, -33.4489],
  },
  {
    level: 5,
    markerOffset: 25,
    name: 'Bogota',
    coordinates: [-74.0721, 4.711],
  },
  {
    level: 5,
    markerOffset: 25,
    name: 'Quito',
    coordinates: [-78.4678, -0.1807],
  },
  {
    level: 5,
    markerOffset: -15,
    name: 'Georgetown',
    coordinates: [-58.1551, 6.8013],
  },
  {
    level: 7,
    markerOffset: -15,
    name: 'Asuncion',
    coordinates: [-57.5759, -25.2637],
  },
  {
    level: 10,
    markerOffset: 25,
    name: 'Paramaribo',
    coordinates: [-55.2038, 5.852],
  },
  {
    level: 3,
    markerOffset: 25,
    name: 'Montevideo',
    coordinates: [-56.1645, -34.9011],
  },
  {
    level: 20,
    markerOffset: -15,
    name: 'Caracas',
    coordinates: [-66.9036, 10.4806],
  },
  {
    level: 6,
    markerOffset: -15,
    name: 'Lima',
    coordinates: [-77.0428, -12.0464],
  },
];

const StyledCard = styled(Card)`
  width: auto !important;
  padding-top: 8px;
`;

// const StyledDiv = styled.div`
//   padding-top: 2rem;
// `;

const MapChart = () => {
  return (
    <div>
      <StyledCard>
        <Card.Header textAlign="center" content="Bản đồ nhiệt" />
        <ComposableMap
          projection="geoAzimuthalEqualArea"
          projectionConfig={{
            rotate: [58, 20, 0],
            scale: 400,
          }}
        >
          <Geographies geography={geoUrl}>
            {({ geographies }) =>
              geographies
                .filter((d) => d.properties.REGION_UN === 'Americas')
                .map((geo) => (
                  <Geography
                    key={geo.rsmKey}
                    geography={geo}
                    fill="#EAEAEC"
                    stroke="#D6D6DA"
                  />
                ))
            }
          </Geographies>
          {markers.map(({ level, name, coordinates, markerOffset }) => (
            <Marker key={name} coordinates={coordinates}>
              <circle r={level} fill="#F00" stroke="#fff" strokeWidth={2} />
              <text
                textAnchor="middle"
                y={markerOffset}
                style={{ fontFamily: 'system-ui', fill: '#5D5A6D' }}
              >
                {name}
              </text>
            </Marker>
          ))}
        </ComposableMap>
      </StyledCard>
    </div>
  );
};

export default MapChart;
