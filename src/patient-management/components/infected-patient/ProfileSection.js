  ...
  const ruleOptions = {
    ...
    immunizations: {
      validate: () => {
        ...
        const immunizationLatestFromFillData =
          fillData && fillData?.immunizations && fillData.immunizations.length > 0
            ? fillData.immunizations.reduce((o, i) => new Date(i.dateCreated) > new Date(o.dateCreated) ? i : o, fillData.immunizations[0])
            : undefined;
      ...
      if (data && data.length > 0) {
        const profile = {
          ...
          addressesInVietnam: data[0]?.addressesInVietnam
            ...
            : [],
          workAddresses: data[0]?.workAddresses
            ? data[0]?.workAddresses
            : [],
          immunizations: data[data.length - 1]?.immunizations
            ? data[data.length - 1]?.immunizations
            : [],
        };
        setFillData(profile);
        reset({
  ...
  useEffect(() => {
    if (conflictSubject?.id || conflictSubject?.fullName) {
      if (!fillData || !isEqualObject(conflictSubject, fillData)) {
        ...
        getExistProfile(conflictSubject?.fullName, conflictSubject?.phoneNumber)
      }
    }
    ...
    if (initialSubject) {
      reset({
        ...
        workAddresses: initialSubject?.workAddresses
          ...
          : [],
        immunizations: initialSubject?.immunizations[initialSubject.length - 1]
          ? initialSubject?.immunizations[initialSubject.length - 1]
          : [],
      });
    } else {
      reset({
        ...
        workAddresses:
          assignQuickTestSession
            ? [{
              ...
              streetHouseNumber: assignQuickTestSession?.streetHouseNumber ?? '',
  ...
  return (
    <Wrapper>
      ...
      <div className="ui form">
          ...
          <Controller
            ...
            render={({ onChange, value }) => (
              <Form.Field
                ...
                fillData={
                  fillData?.immunizations && Array.isArray(fillData.immunizations) && fillData.immunizations.length !== 0
                    ? fillData.immunizations.reduce((o, i) => new Date(i.dateCreated) > new Date(o.dateCreated) ? i : o, fillData.immunizations[0]) // get latest object
                    : {
        ...
        <Accordion as={Form.Field} defaultActiveIndex={isWorkAddressRequired ? 0 : undefined} panels={additionalPanels} />
      </div>
    </Wrapper>
  );
};
